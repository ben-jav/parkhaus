...
function findeAuto(kennzeichen) {
...
function findeAuto1(kennzeichen) {
    for (let etage = 0; etage < parkhaus.length; etage++) {
...
function ausparkenAuto(kennzeichen) {
...
function einparken(kennzeichen, lady) {
    let geparkt = false;
    if (anzahlBelegt < 20) {
        if (lady == 'y') {
...
function anzahlEinfahrtAusfahrt() {
    let anzahl = 0;
    for (let etage = 0; etage < parkhaus.length; etage++) {
        for (let i = 0; i < parkhaus[etage].length; i++) {
